...
::: {style="font-size: 90%;"}
If we have predictor variables like $x_{1,i}$, $x_{2,i}$, ..., $x_{k,i}$ and a binary response variable $y_i$ (where $y_i = 0$ or $y_i = 1$), we need a **"special" function** (or `link function`) able to transform the expected value of the response variable into the outcome we’re trying to predict.
...
+ a (toy) `heart` dataset from the book by: Brian Machut, Nathan Cornwell [Data Analytics with R](https://bookdown.org/brianmachut/uofm_analytics_r_hw_sol_2/#acknowledgements)  
...
We now examine the training dataset `heart_train`, after converting the binary variables `heart_disease` and `coffee_drinker` to factors.
...
#| fig.cap: 'The boxplots indicate that subjects who spend higher amounts annually on fast food show higher likelihood of heart disease (HD =1)'
...
We could technically use linear regression (@fig-lin-reg) to study likelihood of having `Heart Disease` in relation to `fast food spending`, but it would not work:

+ How should we interpret a continuous prediction when it is different from $0$ or $1$ ?
<!-- we know that the response y is categorical, not numeric, so the interpretation of the regression coefficients is strange to say the least: what does it mean when the prediction is less than 0 or greater than 1? If the predictor is supposed to represent likelihood, or probability of having heart disease for a given observation of fast food spending, it should be constrained on [0,1].  -->
<!-- + Second, this is not extensible to multi-class problems that have more than 2 possible categorical response values.  -->
+ Also, here the assumptions for linear model are not met:
    + ***Linear relationship*** between `Y` and `X1`  
...
#| output: true
#| output-location: slide
#| fig.cap: 'Showing that a linear regression (red line) would fit poorly'
...
ggplot(heart_data) +
  ...
  labs(title = "Heart disease [0,1] against fast food spend (US$), with regression line") + 
...
::: {style="font-size: 90%"}
+ We can fit instead a **logistic regression model** to the `heart_train` data using:
  + the `glm` function for Generalized Linear Models; 
    + with argument `family = binomial` to specify the logistic regression model which will use `logit` as the `link` function;
    + and after `~` we specify the predictor variables. 
...
::: {style="font-size: 90%"}
+ @tbl-logit_heart (next) shows the model `summary`, with the `coefficient estimate` for each  predictor.
...
#|    + coefficients'**estimates** are `natural logarithm of the odds` of the event happening (Heart Disease) <br>
#|    + `positive estimate` indicates an increase in the odds of finding a malignant tumor  <br>
#|    + `negative estimate` indicates a decrease in the odds of finding a malignant tumor  <br>
#|    + **odds_ratios** = the exponentiated coefficient estimate  <br>
...
## Interpreting the `logistic regression` model coefficients
:::{style="font-size: 80%"}
...
+ **Intercept**: the estimated intercept is typically not of interest
+ **Coffee Drinker**: Coffee drinking has an estimated coefficient of `-0.7296` which means that the coffee drinking is associated with a decrease in the odds that a person has heart disease. Specifically, it decreases the odds of heart disease (relative to non-coffee drinkers) by a multiplicative factor of `e-0.7296`, or `0.48`, holding all other predictors fixed. Therefore, coffee drinking is also associated with a decrease in the probability of having heart disease. This association appears to be significant.
+ **Fast Food Spend**: A one-unit increase in fast food spend is associated with a `0.0024` increase in the log odds of having heart disease. This association appears to be heavily significant.
+ **Income**: Based on a p-value of `0.8182`, we conclude that income is not significantly associated with heart disease.
:::

## So, is drinking coffee good or bad? 🤔

Our **model** told us that coffee drinking is associated with a `decrease in the likelihood of having heart disease`.  
However, our **plot** above showed that there was a `higher proportion of coffee drinkers with heart disease` as compared to non coffee drinkers. How can that be? <br><br>

It’s because _coffee drinking and fast food spend are correlated_ so, on it’s own, it would appear as if coffee drinking were associated with heart disease, but this is only because coffee drinking is also associated with fast food spend, which our model tells us is the real contributor to heart disease.
