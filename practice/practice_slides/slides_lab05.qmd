...
+ We will review examples of both **supervised** and **unsupervised** ML algorithms
  ...
  + **Unsupervised ML algorithms** examples:
      ...
      + [PLS-DA for classification, a supervised ML alternative to PCA]{style="background-color: #d8cf71;"}
...
:::{aside}
[*highlight* = ]{style="background-color: #d8cf71;"} ![](../../images/coming-soon.png){width=8%}  
:::
:::
...
For simplicity, we will establish a threshold of $0.5$ to classify the predictions as either `Yes_HD` or `No_HD`.

+ using the base function `ifelse(test, yes, no)`
```{r}
#| output: true

# Convert predictions to classifications
heart_train <- heart_train %>%
  mutate(heart_disease_pred_class = ifelse(heart_disease_pred > 0.5, 
                                           "pred_YES", "pred_NO"))
# ... same BASE WAY
...
## Confusion matrix with threshold of 0.5
Once again, we use a $0.5$ threshold to classify the predictions (in `test` data). Then we compare the predicted classification to the actual ones.
...
Sensitivity is really very low, which means that the model is not so good at identifying those who have heart disease. 

  + This is quite problematic in a medical context, where we want to identify as many cases as possible.
  
<!-- This may be due to the distribution of the data or the model itself: people with heart disease are a very small proportion of the dataset, and the model may not have enough information to make accurate predictions. -->

<!-- What if we change the threshold to 0.3 or 0.7? Would the model perform better? -->
::: 
...
```{r}
# Check performance for different thresholds
thresh_025 <- 0.25
thresh_075 <- 0.75
...
A lower threshold ($0.25$) evidently allows to "catch" more of the "TRUE POSITIVES" (51, as opposed to 25 with $0.5$), but will also predict more "FALSE POSITIVES" (56, versus 9 with classification $0.5$ threshold)
...
Not surprisingly, a higher threshold ($0.75$) performs the worst in terms of `sensitivity` 
```{r}
...
+ We use the `pROC` package to plot the **ROC curve**: a graphical representation of the trade-off between the **True Positive Rate (Sensitivity)** and the **False Positive Rate (Specificity)** at various thresholds. 
  + (ideal would go up to the top left corner)
+ The **AUC** is a measure of how well the model can distinguish between classes, with a higher AUC indicating better performance.
+ The **best threshold** is the one that maximizes the **Youden index** (Sensitivity + Specificity - 1).

```{r}
#| output: true
#| output-location: slide

# Create a ROC curve with confidence intervals
roc_curve <- pROC::roc(heart_test_pred$heart_disease,
                       heart_test_pred$heart_disease_pred,
                       ci = TRUE, # Compute confidence intervals
                       of = "thresholds", # Compute AUC
                       thresholds = "best", # Select the best threshold
                       print.thres = "best" # Highlight the best threshold on the plot
)

# Plot the ROC curve with custom color
plot(roc_curve, col = "#00589b", lwd = 2, # Main ROC curve color (blue)
     main = "ROC Curve for Heart Disease Prediction",
     xlab = "(Specificity) False Positive Rate", 
     ylab = "(Sensitivity) True Positive Rate")

# Extract the best threshold value directly from roc_curve
best_threshold <- pROC::coords(roc_curve, 
                         # looks for the best threshold
                         x = "best", 
                         # and returns: 
                         ret=c("threshold","specificity", "sensitivity"))
# Add an horizontpROC::## Add an horizontal line at the best threshold
abline(h = best_threshold$sensitivity, col = "#cd6882", lty = 2)
# Add the AUC text
text(0.5, 0.5, paste("AUC =", round(pROC::auc(roc_curve), 4)), col = "#887455", cex = 1.5)
```
...
Rivedi anche steps in `practice/practice_slides/_biopsy_logistic.qmd`
