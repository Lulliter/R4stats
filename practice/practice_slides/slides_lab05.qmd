...
::: aside
[[*highlight*]{style="background-color: #d8cf71;"} = coming soon]{style="color: #d02e4c;"}
:::
...
```
<br><br>
Better threshold in terms of [TRUE POSITIVES]{style="background: #e9c776"} (`sensitivity` of the test goes from 0.238 to 0.486).
:::
...
```
<br><br>
Better threshold in terms of [TRUE NEGATIVEs]{style="background: #ffc2b0"} (`specificity` of the test goes from 0.996 to 0.999).

:::
  
::: aside
Evidently, the highly imbalanced data (few cases of heart disease) has to do with the low sensitivity of the model.
:::
::::
...
:::{style="font-size: 80%"}
+ The **ROC curve** (_"Receiver Operating Curve"_) represents the trade-off between the **True Positive Rate (Sensitivity)** and the **False Positive Rate (Specificity)** at various thresholds.
...
<!-- Say something about the PCA slides (similar to "Conclusions" slides 54 after Logistic regression) -->
+ Today, we explored just a fraction of the **Machine Learning toolbox**, focusing on two fundamental approaches while learning essential **workflow steps** and **practical tools** that extend to more advanced analyses.
...
+ **`PCA`** represents **Unsupervised Learning**, helping reduce dimensionality, visualize high-dimensional data, and uncover hidden structures.

+ The **context** and the **data** shape analytical choices, whether it's setting a classification **threshold** in Logistic Regression or selecting the **number of components** in PCA to balance information retention and interpretability.
