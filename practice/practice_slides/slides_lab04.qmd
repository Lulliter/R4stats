...
```{r}
#| echo: FALSE
mosquito_nets_v2 <- read_csv(here("practice", "data_input", "04_datasets", "mosquito_nets_v2.csv"))
```
...
# Causal mediation analysis using `mediation` pckg
...
# select variables
  dplyr::select(id = seqn, age, gender, weight, height, race, 
         fv, exerc, pchips, 
...
# Select relevant columns and drop rows with missing values
dat_conf <- dat %>%
  dplyr::select(id, age, bp_systolic, fv, exerc, bmi, smoker, gender, hh_income_usd, ln_hh_income_percap ) %>%
  drop_na() %>% 
  filter(fv <=1500)
...
dat_unadjusted <- broom::augment(model_unadjusted, data = dat_conf) %>%
  dplyr::select( -.resid,  -.hat, -.sigma, -.cooksd, -.std.resid ) %>%
...
## [Recalling some vocabulary]{.r-fit-text}
...
basic_po |> 
  ...
  tab_spanner(
    label = "ITE or \\(\\delta_i\\)", columns = ice, 
